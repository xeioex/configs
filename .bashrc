...
alias install-essential="ainstall -y sshfs gdb linux-tools-2.6.32"
...
function __enter-server() {
    __prepare-server $1
    ssh $1
}

alias prepare-server='__prepare-server'
alias enter-server="__enter-server"
...
#
# HOST only
if [[ $(whoami) == "xeioex" && $(hostname) == "xeioex-host" ]]; then
    ...
    alias build-env-enter="__build-env-enter $BUILDENV"
...
if [[ $(whoami) == "build" || $(whoami) == "root" ]]; then
    alias prepare-build-env="sudo mount -a 2>/dev/null; sudo -u build CHROOTENV=1 bash"
    unset XAUTHORITY
fi
